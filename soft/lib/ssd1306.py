...
#     下面的代码由red润自行封装=========================================
    ...
    def circle(self, x0, y0, r, col=1):
        ...
        self.framebuf.pixel(x0, y0 + r, col)
        self.framebuf.pixel(x0, y0 - r, col)
        self.framebuf.pixel(x0 + r, y0, col)
        self.framebuf.pixel(x0 - r, y0, col)
        ...
        while x < y:
            ...
            self.framebuf.pixel(x0 + y, y0 - x, col)
            self.framebuf.pixel(x0 - y, y0 - x, col)
    def draw_line(self, x1, y1, x2, y2, col=1):
        dx = abs(x2 - x1)
        dy = abs(y2 - y1)
        ...
        err = dx - dy
        while True:
            self.framebuf.pixel(int(x1), int(y1), col)
            if x1 == x2 and y1 == y2:
                break
            e2 = 2 * err
            if e2 > -dy:
            ...
            if e2 < dx:
                err += dx
                y1 += sgn_y
    def draw_triangle(self, x1, y1, x2, y2, x3, y3, col=1):
        self.draw_line(x1, y1, x2, y2, col)
        self.draw_line(x1, y1, x3, y3, col)
